# Process ③ — Take-down / Revert / Incident Response

> Process spec. Condensed quick-reference also in `/RELEASING.md` §Rollback; operational version in the
> `publish` skill (`.claude/skills/publish/references/rollback.md`).
...
[no dependents · <300 downloads/week · single owner]. Consequences are permanent:
`pkg@version` is **burned forever** (can't republish even identical bytes); unpublishing *all* versions
locks the name for **24h**; it **breaks downstream builds** (left-pad). → deprecate + dist-tag + forward-patch
...
- Publish both at the **same version** (lock-step) and have angular depend on core with `^x.y.z` so a
  patched core is auto-adopted. An exact-pinned angular is itself broken → must be patched, not just deprecated.
...
- The bad GitHub Release: **edit** it (banner `⚠️ Broken — superseded by vX`), set **pre-release** (drops the "Latest" badge). Cut a normal Release for the patch.
- Invariant: every npm version has one git tag + one Release; `latest` points at the newest *good* tag.

## Prevention — make rollbacks rare (the highest-leverage habit)

**Soak on `next`, then promote to `latest`** — never publish straight to `latest`:
```bash
npm publish --tag next --provenance --access public        # both packages
# post-publish smoke test against the REAL published tarball, in a clean dir:
...
breakage only shows in a real build). A bad build never becomes the default install.
...
6. Report to npm if malware / out-of-policy takedown needed.
7. Rotate any *other* secret reachable from the build environment.

## Sources
docs.npmjs.com/policies/unpublish · docs.npmjs.com/cli/v11/commands/npm-deprecate · .../npm-dist-tag
