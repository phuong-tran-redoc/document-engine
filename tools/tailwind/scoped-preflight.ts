...
const optionsHandlerForIgnoreAndRemove = (selector: string, { ignore, remove }: Options = {}): string | null => {
...
export const isolateInsideOfContainer: SelectorBasedStrategy<{
  ...
  const whereNotExcept =
    typeof options?.except === 'string' && options.except ? `:where(:not(${options.except},${options.except} *))` : '';
 ...
 */
export const isolateOutsideOfContainer: SelectorBasedStrategy<{ plus?: string }> = (containerSelectors, options) => {
  ...
  const insideOfContainerLogic =
    typeof options?.plus === 'string' && options.plus ? isolateInsideOfContainer(options.plus) : null;
